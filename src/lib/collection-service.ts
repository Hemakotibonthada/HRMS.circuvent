 ...
 *
 * A collection missing from here *and* from `ALLOWED_COLLECTIONS` in
 * `/api/collections/[collection]` is served by nothing: every read and write
 * returns 404, and the page shows an empty state because a failed load and an
 * empty result look identical. That is how payroll shipped unable to run and
 * the holiday calendar shipped permanently empty.
 *
 * `scripts/audit-data-paths.ts` checks this invariant across every dashboard
 * page; it runs in `npm run verify`.
 */
