 ...
 *
 * Lives here rather than in the Android app because it is the same arithmetic
 * on both phones, and because a warning that exists on one platform and not the
 * other is how two apps for one company start disagreeing about somebody's
 * entitlement. [LeaveRules.workingDays] answers the different question of how
 * many days were actually worked; this answers what the balance will lose.
 */
object LeaveCost {
    ...
    fun summarise(
        ...
        return Summary(if (isHalfDay) 0.5 else charged, weekendDays, names)
    }
    ...
    fun describe(summary: Summary): String {
        ...
        if (summary.weekendDays > 0) {
            parts += if (summary.weekendDays == 1) {
                "1 is a weekend day"
            } else {
                "${summary.weekendDays} are weekend days"
            }
        }
